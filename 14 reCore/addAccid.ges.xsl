...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        ...
        <xsl:if test="not($range = ('measure','pitch'))">
            <xsl:message terminate="yes" select="'Value *' || $octave || '* not allowed for param $range. Allowed values are *measure* and *pitch*.'"/>
        </xsl:if>
        ...
        <xsl:if test="//mei:accid">
            <xsl:message terminate="no">
                This file contains one or more &lt;mei:accid&gt; elements, which are not yet supported by this
                stylesheet. It is very likely that the resulting file will be incorrect!
            </xsl:message>
        </xsl:if>
